...
function runBackup {
  ...
  do
    ...
    for d in $(seq 0 ${serverDatabasesLength})
    do
    ...
    if [[ $compressedEnabled == true ]]; then
      ...
      echo -e "${PREFIX} Starting Compressed Backup Process${CYANC}"
        ...
        # Compress folder with Debug
				tar -czvf ${TEMP_FOLDER}${DATE}.tar.gz -C ${serverFolder} .
